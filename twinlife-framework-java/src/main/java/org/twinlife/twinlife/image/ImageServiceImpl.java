...
public class ImageServiceImpl extends BaseServiceImpl<BaseService.ServiceObserver> implements ImageService {
    ...
    private static final boolean ENABLE_UPLOAD = BuildConfig.ENABLE_DATABASE;
    private static final boolean ENABLE_DATABASE = BuildConfig.ENABLE_DATABASE;
    ...
    static class GetImagePendingRequest extends PendingRequest {
        ...
        GetImagePendingRequest(@Nullable ImageId imageId, @NonNull UUID imagePublicId,
    ...
    @Override
    public void getImage(@NonNull UUID imageId, @NonNull ImageService.Kind kind, @NonNull Consumer<Bitmap> consumer) {
        if (DEBUG) {
            Log.d(LOG_TAG, "getImage: imageId=" + imageId + " kind=" + kind);
        }
        ...
        // Get the image from the server.
        long requestId = newRequestId();
        GetImageIQ getImageIQ = new GetImageIQ(IQ_GET_IMAGE_SERIALIZER, requestId, imageId, kind);

        GetImagePendingRequest request = new GetImagePendingRequest(null, imageId, kind, consumer);
        synchronized (mPendingRequests) {
            // Look for the pending requests and if we are already asking for this same image
            // don't make a new request to the server but keep it in the chain.
            for (PendingRequest pendingRequest : mPendingRequests.values()) {
                if (pendingRequest instanceof GetImagePendingRequest) {
                    GetImagePendingRequest imagePendingRequest = (GetImagePendingRequest) pendingRequest;
                    if (imagePendingRequest.kind == kind && imageId.equals(imagePendingRequest.imagePublicId)) {
                        request.nextRequest = imagePendingRequest.nextRequest;
                        imagePendingRequest.nextRequest = request;
                        return;
                    }
    ...
    private void onGetImage(BinaryPacketIQ iq) {
        ...
        if (request.kind == Kind.THUMBNAIL) {
            boolean updated;
            if (ENABLE_DATABASE) {
                updated = mServiceProvider.importImage(request.imageId, request.status, imageData, onGetImageIQ.imageSha);
            } else {
                updated = true;
            }
    ...
    private void onPutImage(@NonNull BinaryPacketIQ iq) {
        ...
        synchronized (mPendingRequests) {
            if (onPutImageIQ.status != OnPutImageIQ.Status.INCOMPLETE) {
                ...
                request = (UploadImagePendingRequest) mPendingRequests.get(requestId);
            }
        }
        if (request == null) {
            return;
    ...
    private void backgroundUpload() {
        ...
        UploadInfo info = mServiceProvider.getNextUpload();
        synchronized (this) {
            mUploadJob = null;
