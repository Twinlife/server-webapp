...
import org.twinlife.twinlife.repository.RemoteRepositoryServiceImpl;
...
public abstract class TwinlifeImpl implements Twinlife, ConnectionListener, BaseServiceProvider {
    ...
    //
    private volatile Connection mWebSocketConnection;
    ...
    public TwinlifeImpl(@NonNull Context context, @NonNull Executor executor) {
        if (DEBUG) {
    ...
    public ErrorCode configure(@NonNull TwinlifeConfiguration twinlifeConfiguration,
        ...
        if (serviceId == null || applicationId == null || twinlifeConfiguration.applicationName == null
                || twinlifeConfiguration.applicationVersion == null
                || twinlifeConfiguration.certificateSerialNumber == null
                || twinlifeConfiguration.apiKey == null) {

            if (Logger.ERROR) {
                Logger.error(LOG_TAG, "invalid application configuration");
            }
        ...
        mWebSocketConnection.setConnectionListener(this);
        ...
        mManagementServiceImpl = new ManagementServiceImpl(this, mWebSocketConnection);
        ...
        mRepositoryServiceImpl = new RemoteRepositoryServiceImpl(this, mWebSocketConnection, twinlifeConfiguration.factories);
    ...
    public boolean isConnected() {
        ...
        return mWebSocketConnection != null && mWebSocketConnection.isConnected();
    }
    ...
    public long getServerTimeCorrection() {
        if (DEBUG) {
            Log.d(LOG_TAG, "getServerTimeCorrection");
        }
    ...
    protected int connectInternal() {
        ...
        // Default retry in 10s.
        int result = 10 * 1000;
        try {
            webSocketConnection.connect();

        //} catch (WebSocketContainer.UnknownHostException unknownHostException) {
            // DNS failed, retry in 5s (it could be a transient failure).
        //    result = 5 * 1000;

        //} catch (WebSocketContainer.SSLException | WebSocketContainer.HandshakeException sslException) {
            // SSL failed, retry in 1mn (in most cases it is a persistent failure).
        //    result = 60 * 1000;

        } catch (Exception exception) {
            if (Logger.INFO) {
                Logger.info(LOG_TAG, "WebSocket connect error: ", exception);
            }
        }

        final boolean connected = webSocketConnection.isConnected();
        if (connected) {
            result = 0;
            onConnect();
        } else {
